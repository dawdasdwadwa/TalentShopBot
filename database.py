...
async def get_user_active_ticket(user_id: int) -> Optional[dict]:
    row = await fetchone('SELECT * FROM tickets WHERE user_id = ? AND status = "open" LIMIT 1', (user_id,))
    return dict(row) if row else None

async def close_ticket(channel_id: int):
    closed_at = datetime.now(timezone.utc).isoformat()
    async with transaction():
        await _execute_no_lock('UPDATE tickets SET status = "closed", closed_at = ? WHERE channel_id = ?', (closed_at, channel_id))
...
async def get_seller_reviews(seller_id: int, limit: int = 10) -> List[dict]:
    rows = await fetchall('SELECT * FROM reviews WHERE seller_id = ? ORDER BY created_at DESC LIMIT ?', (seller_id, limit))
    return [dict(row) for row in rows] if rows else []

async def get_user_reviews(user_id: int) -> List[dict]:
    rows = await fetchall('SELECT * FROM reviews WHERE user_id = ? ORDER BY created_at DESC', (user_id,))
    return [dict(row) for row in rows] if rows else []
...
async def get_blacklist() -> List[int]:
    ...
    blacklist_cache = [r['user_id'] for r in rows]
    return blacklist_cache
...
async def get_user_warnings(user_id: int) -> List[Warning]:
    rows = await fetchall('SELECT id, moderator_id, reason, created_at FROM warnings WHERE user_id = ?', (user_id,))
    return [
        Warning(id=r['id'], user_id=user_id, moderator_id=r['moderator_id'], reason=r['reason'], created_at=r['created_at'])
        for r in rows
    ]
...
async def get_shop_messages(guild_id: int) -> dict:
    ...
    return {"img": row['img_id'], "stat": row['stat_id']} if row else {}
...
async def get_currency_rates() -> Dict[str, float]:
    ...
    currency_rates_cache = {row['currency']: row['rate'] for row in rows}
    return currency_rates_cache
                ...
                if attachment.filename.endswith('.json') and ('shop_backup_' in attachment.filename or 'scheduled_backup_' in attachment.filename):
                    ...
                    async with transaction():
                        ...
                        for cat_data in categories_data.values():
                            await _execute_no_lock(
                                'INSERT INTO categories (name, emoji, description, image_url) VALUES (?, ?, ?, ?)',
                                (cat_data['name'], cat_data.get('emoji', '📁'), cat_data.get('description'), cat_data.get('image_url'))
                            )
                        for lot_data in lots_data.values():
                            await _execute_no_lock(
                                ...
                                (lot_data['name'], lot_data.get('price', ''),
                                 ...
                                 lot_data.get('stock', 0), lot_data.get('image_url'), lot_data.get('role_id'))
                            )
                    await refresh_cache()
